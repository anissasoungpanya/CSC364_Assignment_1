...
def start_server():
    ...
    router6_table_path = os.path.join(BASE_DIR, "input", "router_6_table.csv")

    try:
        forwarding_table = read_csv(router6_table_path)
    except FileNotFoundError:
        print(f"[Router 6] Missing forwarding table at: {router6_table_path}")
        sys.exit(1)
...
def processing_thread(connection, ip, port, forwarding_table_with_range, default_gateway_port, max_buffer_size=5120):
    discarded_path = "./output/discarded_by_router_6.txt"
    out_path = "./output/out_router_6.txt"
    ...
    while True:
        ...
        # 11. Either
        # (a) append the payload to out_router_6.txt without forwarding because this router is the last hop, or
        # (b) append the new packet to discarded_by_router_6.txt and do not forward the new packet
