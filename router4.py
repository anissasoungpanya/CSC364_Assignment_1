...
def start_server():
    # 1. Create a socket.
    host = "127.0.0.1"
    port = 8004 #Router 4 listens here
    soc = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    soc.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    ...
    router4_table_path = os.path.join(BASE_DIR, "input", "router_4_table.csv")

    try:
        forwarding_table = read_csv(router4_table_path)
    except FileNotFoundError:
        print(f"[Router 4] Missing forwarding table at: {router4_table_path}")
        sys.exit(1)
...
def processing_thread(connection, ip, port, forwarding_table_with_range, default_gateway_port, max_buffer_size=5120):
    ...
    sock_to_r5 = create_socket("127.0.0.1", 8005)
    sock_to_r6 = create_socket("127.0.0.1", 8006)

    sent_path = "./output/sent_by_router_4.txt"
    discarded_path = "./output/discarded_by_router_4.txt"
    out_path = "./output/out_router_4.txt"
    ...
    while True:
        ...
        if send_interface == "8005":
            print("sending packet", new_packet, "to Router 5")
            write_to_file(sent_path, new_packet, "5")
            try:
                sock_to_r5.sendall((new_packet + "\n").encode("utf-8"))
            except Exception:
                write_to_file(discarded_path, new_packet)
        elif send_interface == "8006":
            print("sending packet", new_packet, "to Router 6")
            write_to_file(sent_path, new_packet, "6")
            try:
                sock_to_r6.sendall((new_packet + "\n").encode("utf-8"))
            except Exception:
                write_to_file(discarded_path, new_packet)
        elif send_interface == "127.0.0.1":
            print("OUT:", payload)
