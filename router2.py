...
def start_server():
    # 1. Create a socket.
    host = "127.0.0.1"
    port = 8002 #Router 2 listens hre
    soc = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    soc.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    ...
    router2_table_path = os.path.join(BASE_DIR, "input", "router_2_table.csv")

    try:
        forwarding_table = read_csv(router2_table_path)
    except FileNotFoundError:
        print(f"[Router 2] Missing forwarding table at: {router2_table_path}")
        sys.exit(1)
...
def processing_thread(connection, ip, port, forwarding_table_with_range, default_gateway_port, max_buffer_size=5120):
    ...
    sock_to_r3 = create_socket("127.0.0.1", 8003)
    sock_to_r4 = create_socket("127.0.0.1", 8004)

    sent_path = "./output/sent_by_router_2.txt"
    discarded_path = "./output/discarded_by_router_2.txt"
    out_path = "./output/out_router_2.txt"
    ...
    while True:
        ...
        if send_interface == "8003":
            print("sending packet", new_packet, "to Router 3")
            write_to_file(sent_path, new_packet, "3")
            try:
                sock_to_r3.sendall((new_packet + "\n").encode("utf-8"))
            except Exception:
                write_to_file(discarded_path, new_packet)
        elif send_interface == "8004":
            print("sending packet", new_packet, "to Router 4")
            write_to_file(sent_path, new_packet, "4")
            try:
                sock_to_r4.sendall((new_packet + "\n").encode("utf-8"))
            except Exception:
                write_to_file(discarded_path, new_packet)
